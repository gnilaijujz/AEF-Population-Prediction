...
def resolve_mlp_input_csv(
    ...
    if input_csv:
        resolved = Path(input_csv)
    else:
        feature_name = safe_name(input_feature)
        resolved = Path(input_root) / city / f"{city}__AEF64_plus_{feature_name}.csv"
    if not resolved.exists():
        raise FileNotFoundError(f"MLP input CSV does not exist: {resolved}")
    return resolved

def load_mlp_input_csv(input_csv: Path) -> Tuple[pd.DataFrame, List[str]]:
    # MODIFIED: directly read the prepared 65-dim input CSV: AEF64 + one GIS feature.
    df = pd.read_csv(input_csv, dtype={"cb_2020_3": str, "GEO_ID": str})
    aef_cols = [f"A{i:02d}" for i in range(64)]
    missing_aef = [col for col in aef_cols if col not in df.columns]
    if missing_aef:
        raise ValueError(f"{input_csv} is missing AEF columns: {missing_aef}")
    if "population_density" not in df.columns:
        raise ValueError(f"{input_csv} must contain population_density.")

    aux_cols = {
        "cb_2020_3", "GEO_ID", "NAME", "MSA_NAME", "AEF_SOURCE",
        "B01003_001E", "population", "tract_area_sqkm",
        "population_density", "y_log_density",
    }
    gis_cols = [col for col in df.columns if col not in aux_cols and col not in aef_cols]
    if len(gis_cols) < 1:
        raise ValueError(f"{input_csv} should contain at least one extra GIS/NTL feature column.")

    feature_cols = [*aef_cols, *gis_cols]
    for col in feature_cols + ["population_density"]:
        df[col] = pd.to_numeric(df[col], errors="coerce")
    if "population" not in df.columns and "B01003_001E" in df.columns:
        df["population"] = pd.to_numeric(df["B01003_001E"], errors="coerce")
    if "GEO_ID" not in df.columns and "cb_2020_3" in df.columns:
        df["GEO_ID"] = df["cb_2020_3"].astype(str)

    df = df.dropna(subset=feature_cols + ["population_density"]).reset_index(drop=True)
    if df.empty:
        raise ValueError(f"{input_csv} has no usable rows after dropping missing values.")

    print(f"MLP input CSV: {input_csv}")
    print(f"Rows: {len(df):,}")
    print(f"Feature dimensions: {len(feature_cols)} = AEF64 + {len(gis_cols)} extra features")
    print(f"Extra feature columns: {gis_cols}")
    print("Target: population_density = population / tract_area_sqkm (people per sq km)")
...
def discover_mlp_input_cities(input_root: Path, input_feature: str) -> List[str]:
    feature_name = safe_name(input_feature)
    cities = []
    for city_dir in sorted(p for p in input_root.iterdir() if p.is_dir()):
        expected = city_dir / f"{city_dir.name}__AEF64_plus_{feature_name}.csv"
        if expected.exists():
            try:
                has_rows = len(pd.read_csv(expected, usecols=[0], nrows=1)) > 0
            except Exception:
                has_rows = False
            if has_rows:
...
def train_single_city(city: str, args: argparse.Namespace, out_dir: Path) -> None:
    ...
    if args.input_mode == "mlp_input":
        # MODIFIED: use prepared 65-dim CSV from MLP_input instead of rebuilding AEF + population + shp.
        input_csv = resolve_mlp_input_csv(args.input_root, city, args.input_feature, args.input_csv)
        gdf, feature_cols = load_mlp_input_csv(input_csv)
    else:
    ...
    feature_suffix = f"_{safe_name(args.input_feature)}" if args.input_mode == "mlp_input" else ""
    output_stem = f"{city}{feature_suffix}_MLP"
    ...
    if "geometry" in gdf.columns:
        ...
        print("Input CSV has no geometry column; skipping prediction shapefile.")
    ...
    try:
        ...
        if len(df_hist) >= 2:
            ...
            ax2.set_ylabel("RMSE (people/km2)")
            ax2.set_title("RMSE")
            ax2.legend()
            ax2.grid(True)

            ax3 = axes[2]
            ax3.plot(df_hist["epoch"], df_hist["train_r2"], label="Train R2", color="green")
            ax3.plot(df_hist["epoch"], df_hist["val_r2"], label="Val R2", color="red", linestyle="--")
            ...
            print("Too few history points; skipping loss curve plot.")
    except ImportError:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--input-csv", default=None,
                        help="Optional direct 65-dim input CSV for a single city.")
...
def main() -> None:
    ...
    if args.all:
        if args.input_mode == "mlp_input":
            cities = discover_mlp_input_cities(Path(args.input_root), args.input_feature)
        else:
