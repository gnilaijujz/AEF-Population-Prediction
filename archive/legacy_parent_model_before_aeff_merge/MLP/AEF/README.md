# archive\legacy_parent_model_before_aeff_merge\MLP\AEF

## Purpose

旧版本代码、历史实验输出和非主流程文件，仅作参考。

## Contents

- Subdirectories: 1
- Files: 2
- Key subdirectories: `transfer_results_log_mlp/`

## Code

- `MLP_regression.py`: 回归建模或回归分析脚本。
- `MLP_transfer.py`: 跨城市迁移实验或迁移能力分析脚本。

## Path Notes

- Local project paths are relative to the repository root.
- Generated outputs should stay under `results/` or `validation/results/`; reusable inputs should stay under `data_sources/` or `model_data/`.
