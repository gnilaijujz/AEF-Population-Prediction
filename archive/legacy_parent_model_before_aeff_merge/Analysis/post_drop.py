...
# -------------------------- 配置 --------------------------
# 请根据实际文件名修改
R2_MATRIX_FILE = r"results/transfer_results/aef/transfer_matrix_r2.csv"   # 校准后零模型 R² 矩阵
EMBEDDING_DIST_FILE = r"results/transfer_results/aef/transfer_embedding_distances_gnn.csv"  # 可选，格式见下文
...
outlier_df.to_csv(f"results/legacy_analysis/iqr/{OUTPUT_PREFIX}_outlier_pairs.csv", index=False)
print(f"异常城市对列表已保存至 results/legacy_analysis/iqr/{OUTPUT_PREFIX}_outlier_pairs.csv")
...
summary_df.to_csv(f"results/legacy_analysis/iqr/{OUTPUT_PREFIX}_summary_stats.csv")
print("\n统计摘要:")
print(summary_df)
...
if has_embedding and "EmbeddingDistance" in df_long.columns:
    ...
    reg_results.to_csv(f"results/legacy_analysis/iqr/{OUTPUT_PREFIX}_regression_results.csv", index=False)
...
plt.tight_layout()
plt.savefig(f"results/legacy_analysis/iqr/{OUTPUT_PREFIX}_heatmap.png", dpi=300, bbox_inches="tight")
plt.close()
...
if has_embedding and "EmbeddingDistance" in df_long.columns:
    ...
    plt.tight_layout()
    plt.savefig(f"results/legacy_analysis/iqr/{OUTPUT_PREFIX}_scatter.png", dpi=300, bbox_inches="tight")
    plt.close()
...
if len(outlier_pairs) > 0:
    ...
    plt.tight_layout()
    plt.savefig(f"results/legacy_analysis/iqr/{OUTPUT_PREFIX}_outlier_table.png", dpi=300, bbox_inches="tight")
    plt.close()
