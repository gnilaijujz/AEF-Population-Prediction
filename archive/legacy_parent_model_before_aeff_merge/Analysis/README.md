...
| `post_drop.py` | 对 **null 模型**校准 R² 矩阵做 IQR 异常剔除、汇总、热图/散点 | `null_calibrated_*`(在 `results/legacy_analysis/iqr/`) |
| `post_drop_sec.py` | `post_drop` 的推广版,一次跑 `null` 与 `gnn` 两个矩阵 | `transfer_analysis_<mode>_*` |
| `post_process_attribute.py` | 域距离↔校准 R² 的归因(相关 + IQR 前后 + 诊断图) | `attribution_*`(在 `results/legacy_analysis/attribute/`) |
...
- 训练/迁移默认写到 `results/legacy_analysis/output_try_log`、`results/legacy_analysis/transfer_results_try_log`;
- 但分析脚本读的是 `results/legacy_analysis/transfer_results_experiments/`、`results/legacy_analysis/mlp_transfer_results_1/`,并写入需要**预先创建**的 `results/legacy_analysis/iqr/`、`results/legacy_analysis/attribute/`。
