...
├── README.md
├── .gitignore
├── processed_aef/                     # 纯 AEF 输入:每 MSA 的 64 维嵌入(aef_*_b*_2020.csv)+ 边界
├── processed_gis/                     # 纯 GIS 输入:每 MSA 的 GIS 特征(gis_*_b0_2020.csv)+ 边界
│   ├── create_dataset.ipynb      #   GIS 数据集构建流程
│   └── feature_corr.ipynb        #   特征相关性分析
├── MSA/                          # 研究区(见 MSA/README.md)
│   └── selected_15_msa/                    #   高/中/低分层抽样脚本 + selected_15_msa 名单 / tract 清单 / 人口标签
├── raw_gis_features/               # 各特征的提取代码 + 每个数据集文件夹的 README
│   ├── selected_15_msa_features/                    #   处理后 selected_15_msa 特征表(dem/ndvi/nlcd/poi/pop/socioeconomic/tcc/water/building/ntl/road)
│   ├── 社会经济数据/              #   NHGIS / 人口结构
