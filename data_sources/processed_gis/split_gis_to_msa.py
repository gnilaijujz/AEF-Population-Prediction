...
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))   # processed_gis/
ROOT = os.path.dirname(SCRIPT_DIR)                        # data_sources root
AEF_ROOT = os.path.join(ROOT, "processed_aef")
STD_CSV = os.path.join(ROOT, "raw_gis_features", "selected_15_msa_features", "selected_9_features_15msa_standardized.csv")
...
for shp in shps:
    ...
    print(f"  {msa:14s}  shp_tracts={n_shp:5d}  kept={len(keep):5d}  shp:{shp_note:12s} -> {os.path.relpath(out_csv, ROOT)}")
