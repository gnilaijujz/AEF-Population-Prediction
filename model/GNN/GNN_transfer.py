...
# ---------------------------- 配置 ---------------------------------
DEFAULT_AEF_ROOT = Path(r"assets/data_AEF")
DEFAULT_POP_CSV = Path(r"assets/ACSDT5Y2020.pop/ACSDT5Y2020.B01003-Data.csv")
DEFAULT_PRETRAINED_DIR = Path("GNN_output/output_try_log")
DEFAULT_OUT_DIR = Path("GNN_output/transfer_results_try_log")
...
def discover_cities(aef_root: Path) -> List[str]:
    ...
    if not cities:
        raise ValueError(f"No city folders with both AEF CSV files and shapefile found under {aef_root}")
    return cities
...
def predict_density(model, data, y_mean, y_std, target_transform) -> np.ndarray:
    model.eval()
    with torch.no_grad():
...
def run_transfer(args: argparse.Namespace) -> None:
    ...
    for source, src_bundle in source_models.items():
        ...
        for target in target_cities:
            ...
            y_pred = predict_density(model, target_data, y_mean, y_std, target_transform)
