...
DEFAULT_PRETRAINED_DIR = Path("MLP_output")
DEFAULT_OUT_DIR = Path("MLP_output/transfer_results")
...
def load_city(aef_root: Path, city: str, pop_csv: Path):
    ...
    aef_dir = mlp.resolve_aef_dir(str(aef_root), city, None)
    shp_path = mlp.resolve_shp_path(aef_dir, None)
    gdf, feature_cols = mlp.load_and_merge_data(aef_dir, pop_csv, shp_path)
