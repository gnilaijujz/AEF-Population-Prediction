...
DEFAULT_AEF_ROOT = Path(r"model_data/aef_root/aef_plus_nighttime_lights")
DEFAULT_POP_CSV = Path(r"data_sources/msa_reference/raw_msa_data/ACSDT5Y2020.pop/ACSDT5Y2020.B01003-Data.csv")
DEFAULT_PRETRAINED_DIR = Path(r"model_data\pretrained_models\GNN\selftrain_gnn_plus_nighttime_lights")
DEFAULT_OUT_DIR = Path(r"results/transfer_results/aef_plus_nighttime_lights")
GIS_COLS = ['ntl_sum']   # 您要添加的 GIS 特征列
...
def load_city(aef_root: Path, city: str, pop_csv: Path, gis_cols: List[str] = None):
    if gis_cols is None:
        gis_cols = GIS_COLS
    ...
    # 检查 gdf 中是否已有 GIS 列
    missing_gis = [col for col in gis_cols if col not in gdf.columns]
    if missing_gis:
        # 尝试从原始 CSV 加载
        aef_files = sorted(aef_dir.glob("aef_*_b*_2020.csv"))
        parts = []
        for f in aef_files:
            df_temp = pd.read_csv(f, dtype={"TRACT_ID": str})
            keep_cols = ['TRACT_ID'] + [col for col in missing_gis if col in df_temp.columns]
            if keep_cols:
                parts.append(df_temp[keep_cols].drop_duplicates("TRACT_ID"))
        if parts:
            df_gis = pd.concat(parts, ignore_index=True).drop_duplicates("TRACT_ID")
            df_gis['TRACT_ID'] = df_gis['TRACT_ID'].astype(str).str.strip()
            gdf['GEO_ID'] = gdf['GEO_ID'].astype(str).str.strip()
            gdf = gdf.merge(df_gis, left_on='GEO_ID', right_on='TRACT_ID', how='left')
            for col in missing_gis:
            ...
            if 'TRACT_ID' in gdf.columns:
                gdf = gdf.drop(columns=['TRACT_ID'])
        else:
            # CSV 中没有这些列，直接添加0列
            for col in missing_gis:
                gdf[col] = 0.0
    # 确保所有 GIS 列都出现在 feature_cols 中
    for col in gis_cols:
...
def run_single_transfer(
    ...
    if use_ridge:
        ...
        for src, src_bundle in source_bundles.items():
            ...
            # ---- 提取源域嵌入（只需一次） ----
            src_gdf, src_feat_cols, src_edge_index = city_cache[src]
            # 构建源域数据（使用模型的标准化参数）
            src_data, _ = prepare_target_data(
                src_gdf,
                feat_cols,          # 使用模型的 feat_cols，与源域一致
                src_edge_index,
                x_scaler,
                y_mean,
                y_std,
            ...
            with torch.no_grad():
                src_emb = model.forward_emb(src_data.x, src_data.edge_index).detach().cpu().numpy()
...
def run_experiments(args: argparse.Namespace) -> None:
    ...
    for src in model_cities:
        if src not in city_cache:
            print(f"⚠️  City data for {src} not loaded, skipping its model.")
            continue
