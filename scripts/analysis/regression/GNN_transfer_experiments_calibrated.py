...
DEFAULT_AEF_ROOT = Path(r"model_data/aef_root/aef_plus_gis9")
DEFAULT_POP_CSV = Path(r"data_sources/msa_reference/raw_msa_data/ACSDT5Y2020.pop/ACSDT5Y2020.B01003-Data.csv")
DEFAULT_PRETRAINED_DIR = Path(r"model_data\pretrained_models\GNN\selftrain_gnn_plus_gis9")
DEFAULT_OUT_DIR = Path(r"results/transfer_results/aef_plus_gis9")
GIS_COLS = ['impervious_mean',	'building_density',	'poi_total_all',	'poi_diversity',
            'ntl_sum',	'road_density_local',	'ndvi_mean',	'slope_mean',	'dist_water_mean']   # 您要添加的 GIS 特征列
...
def discover_cities(aef_root: Path) -> List[str]:
    ...
    for city_dir in sorted(p for p in aef_root.iterdir() if p.is_dir()):
        has_aef = any(city_dir.glob("gis_*.csv"))   # 改为 gis_*.csv
