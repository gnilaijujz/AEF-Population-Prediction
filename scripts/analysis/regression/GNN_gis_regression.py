...
'''
def load_aef_features(aef_dir: Path) -> Tuple[pd.DataFrame, List[str]]:
    # MODIFIED: use the last 65 columns as model inputs:
    # AEF 64 dimensions + the appended GIS feature column.
    feature_count = 9
    metadata_cols = {"TRACT_ID", "MSA_NAME", "AEF_SOURCE"}

    aef_files = sorted(aef_dir.glob("gis_*.csv"))
    if not aef_files:
        raise ValueError(f"No AEF files matching aef_*_b*_2020.csv found in {aef_dir}")

    first_header = pd.read_csv(aef_files[0], nrows=0)
    candidate_cols = [col for col in first_header.columns if col not in metadata_cols]

    if len(candidate_cols) < feature_count:
        raise ValueError(
            f"{aef_files[0]} has only {len(candidate_cols)} candidate feature columns, "
            f"but {feature_count} are required."
        )

    feature_cols = candidate_cols[-feature_count:]
...
def inverse_target(y_model: np.ndarray, mode: str) -> np.ndarray:
    ...
    if mode == "log1p":
        ...
        # 裁剪 exp 输入以防止 expm1 溢出（exp(700) ≈ 1e304，仍可能使平方溢出）
        y_model_clipped = np.clip(y_model, -700, 700)
        result = np.expm1(y_model_clipped)
        # 根据实际人口密度上限裁剪（这里设为 1e6，可根据实际情况调整）
        result = np.clip(result, 0, 1e6)
...
def discover_cities(aef_root: Path) -> List[str]:
    ...
    for city_dir in sorted(p for p in aef_root.iterdir() if p.is_dir()):
        has_aef = any(city_dir.glob("gis_*.csv"))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--batch-size", type=int, default=256, help="Mini-batch size for NeighborLoader.")
