...
GIS_COLS = ['impervious_mean',	'building_density',	'poi_total_all',	'poi_diversity',
            'ntl_sum',	'road_density_local',	'ndvi_mean',	'slope_mean',	'dist_water_m']   # 您要添加的 GIS 特征列
...
DEFAULT_aef_ROOT = Path(r"model_data/aef_root/aef_plus_building")
...
def load_aef_features(aef_dir: Path) -> Tuple[pd.DataFrame, List[str]]:
    ...
    for aef_file in aef_files:
        ...
        # 保留所有列（不再只选取 TRACT_ID + A 系列）
        aef_part = aef_part.drop_duplicates("TRACT_ID")
    ...
    # 检查 A 系列特征是否存在（用于返回特征列表）
    aef_feature_cols = [f"A{i:02d}" for i in range(64)]
    missing = [col for col in aef_feature_cols if col not in aef.columns]
    if missing:
        raise ValueError(f"AEF files missing feature columns: {missing}")
    ...
    print(f"aef feature dimensions: {len(aef_feature_cols)}")
    return aef, aef_feature_cols
...
def load_and_merge_data(
    ...
    # 1. 加载 AEF（现在包含所有列）
    aef, aef_feature_cols = load_aef_features(aef_dir)
    ...
    aef = aef.drop_duplicates("GEO_ID")

    merged = gdf.merge(aef, on="GEO_ID", how="inner").merge(pop, on="GEO_ID", how="inner")

    # 6. 检查 GIS 列是否存在
    missing_gis = [col for col in GIS_COLS if col not in merged.columns]
    if missing_gis:
        # 尝试模糊匹配（忽略大小写/空格）并重命名
        merged_cols_lower = {col.strip().lower(): col for col in merged.columns}
        for std_col in GIS_COLS:
            lower_std = std_col.lower()
            if lower_std in merged_cols_lower:
                actual = merged_cols_lower[lower_std]
                if actual != std_col:
                    merged = merged.rename(columns={actual: std_col})
                    print(f"Renamed column: '{actual}' -> '{std_col}'")
            else:
                # 如果仍然找不到，抛出明确错误
                raise ValueError(f"GIS column '{std_col}' not found in merged data. Available columns (partial): {list(merged.columns)[:10]}...")

    # 7. 组合特征列（AEF + GIS）
    feature_cols = aef_feature_cols + GIS_COLS
    ...
    if merged.empty:
        raise ValueError("Merged dataset is empty. Check GEOID formats across all sources.")
    ...
    print(f"Total features: {len(feature_cols)} (AEF: {len(aef_feature_cols)}, GIS: {len(GIS_COLS)})")
