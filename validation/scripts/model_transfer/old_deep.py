...
CDS_MATRIX = Path(r"validation\results\transferability\transfer_loss_matrix_robust_entropy.csv")
OUTPUT_DIR = Path(r"validation\results\embedding_health_deep_analysis")
...
# ======================== 3. 加载迁移性能数据（仅针对旧城市目标） ========================
print("\n" + "=" * 70)
print("3. 加载迁移损失矩阵并计算平均损失（只保留到旧城市的迁移）")
print("=" * 70)

df_cds = pd.read_csv(CDS_MATRIX, index_col=0)

# 新城市列表（即行索引）
new_cities = list(df_cds.index)
# 目标列名
all_targets = list(df_cds.columns)
# 旧城市 = 列名不在新城市列表中的
old_targets = [col for col in all_targets if col not in new_cities]

print(f"新城市（源）: {new_cities}")
print(f"旧城市（目标）: {old_targets}")

# 计算每个源城市到所有旧城市的平均迁移损失（排除 NaN）
avg_CDS_old = {}
for src in df_cds.index:
    row = df_cds.loc[src]
    # 提取到旧目标的损失值
    old_losses = row[old_targets].dropna().values
    if len(old_losses) > 0:
        avg_CDS_old[src] = np.mean(old_losses)
    else:
        avg_CDS_old[src] = np.nan

# （可选）保留自预测 R²，但不用于主要分析
r2_self = {src: df_cds.loc[src, src] for src in df_cds.index if src in df_cds.columns and not np.isnan(df_cds.loc[src, src])}

# 合并到健康度数据框
df_health['avg_CDS_old'] = df_health.index.map(lambda c: avg_CDS_old.get(c, np.nan))
# 如果您还想保留自 R²，可以添加
df_health['R2_self'] = df_health.index.map(lambda c: r2_self.get(c, np.nan))

# 删除没有旧城市迁移损失数据的城市（如果有）
df_health = df_health.dropna(subset=['avg_CDS_old'])
print(f"有效城市（有迁移到旧城市的性能数据）: {len(df_health)}")
...
metric_cols = ['effective_rank', 'isotropy', 'inter_intra_ratio', 'neighbor_consistency', 
               'pca_5_explained', 'n_nodes',  'embedding_diversity']
...
# 相关性矩阵
corr = df_health[metric_cols + ['avg_CDS_old', 'R2_self']].corr()
print("\n相关性矩阵（与avg_CDS_old）:")
print(corr['avg_CDS_old'].sort_values(ascending=False))

# 单变量回归
univariate_results = []
for col in metric_cols:
    X = sm.add_constant(df_scaled[[col]])
    model = sm.OLS(df_scaled['avg_CDS_old'], X).fit()
...
# 5.2 有效秩 vs avg_CDS_old 散点图
fig, ax = plt.subplots(figsize=(10, 6))
for city in df_health.index:
    ax.scatter(df_health.loc[city, 'effective_rank'], df_health.loc[city, 'avg_CDS_old'], 
               s=100, alpha=0.7, color='steelblue')
    ax.annotate(city, (df_health.loc[city, 'effective_rank'], df_health.loc[city, 'avg_CDS_old']),
                fontsize=9, xytext=(5, 5), textcoords='offset points')
# 回归线
X = sm.add_constant(df_health[['effective_rank']])
model = sm.OLS(df_health['avg_CDS_old'], X).fit()
...
ax.set_ylabel('平均迁移损失 (avg_CDS_old)')
...
# 选择前3个和后3个城市
sorted_by_cds = df_health.sort_values('avg_CDS_old')
...
for city in sample_cities:
    ...
    ax.plot(range(1, len(cumsum)+1), cumsum, 'o-', linewidth=2, label=f"{city} (CDS={df_health.loc[city, 'avg_CDS_old']:.3f})")
ax.set_xlabel('主成分数量')
...
with open(OUTPUT_DIR / "analysis_summary.txt", 'w') as f:
    ...
    f.write("1. 相关性分析（与avg_CDS_old）:\n")
    for metric in metric_cols:
        r = corr.loc[metric, 'avg_CDS_old']
