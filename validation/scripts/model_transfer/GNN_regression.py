...
def load_aef_features(aef_dir: Path) -> Tuple[pd.DataFrame, List[str]]:
    ...
    aef_files = sorted(aef_dir.glob("aef_*.csv"))
    if not aef_files:
        raise ValueError(f"No aef files matching aef_*.csv found in {aef_dir}")

    parts = []
    for aef_file in aef_files:
        # 1. 先读取，不指定 dtype
        aef_part = pd.read_csv(aef_file)

        # 2. 自动查找 ID 列（支持多种命名）
        id_col = None
        for col in aef_part.columns:
            if col.upper() in ["GEOID", "TRACT_ID", "GEO_ID"]:
                id_col = col
                break
        if id_col is None:
            raise ValueError(f"{aef_file} must contain a tract ID column (GEOID, GEO_ID, or TRACT_ID).")
        if id_col != "TRACT_ID":
            aef_part = aef_part.rename(columns={id_col: "TRACT_ID"})

        # 3. 确保为字符串，并补齐前缀
        aef_part["TRACT_ID"] = aef_part["TRACT_ID"].astype(str)
        if not aef_part["TRACT_ID"].str.startswith("1400000US").all():
            aef_part["TRACT_ID"] = "1400000US" + aef_part["TRACT_ID"]
...
def load_and_merge_data(
    ...
    merged = merged.dropna(subset=["population_density"]).reset_index(drop=True)
    # 在 merged = merged.dropna(subset=["population_density"]).reset_index(drop=True) 之后添加：

    # 1. 过滤面积过小的 tracts（例如 < 0.01 km²）
    min_area = 0.00001  # 可根据实际情况调整
    merged = merged[merged["tract_area_sqkm"] > min_area]

    # 2. 过滤密度过大的 tracts（例如 > 1,000,000 人/km²）
    max_density = 1e6  # 市区人口密度通常不超过 10 万，此处放宽
    merged = merged[merged["population_density"] < max_density]
...
def train_single_city(city: str, args: argparse.Namespace, out_dir: Path) -> None:
    ...
    # 在 train_single_city 中，计算 y_raw 后：
    abnormal = gdf[(gdf["tract_area_sqkm"] < 1e-6) | (gdf["population_density"] > 1e10)]
    print(abnormal[["GEO_ID", "tract_area_sqkm", "population", "population_density"]])
...
def discover_cities(aef_root: Path) -> List[str]:
    ...
    for city_dir in sorted(p for p in aef_root.iterdir() if p.is_dir()):
        # 修改 glob 模式：匹配所有以 aef_ 开头、以 _2020.csv 结尾的文件
