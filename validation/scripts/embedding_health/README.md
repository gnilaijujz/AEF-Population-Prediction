# validation/scripts/embedding_health
...
- Subdirectories: 0
- Files: 8
- Key files: `GNN_regression.py`, `GNN_transfer_experiments_calibrated.py`, `compute_new_metrics.py`, `draw.py`, `embedding_health_analysis.py`, `embedding_health_deep_analysis.py`, `old_deep.py`, `old_embedding.py`
...
- `compute_new_metrics.py`: 新城市嵌入健康度计算（基于预训练模型和数据） 完全复用 old_deep.py 的加载逻辑，只计算纯几何指标。
