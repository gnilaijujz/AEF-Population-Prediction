...
OUTPUT_ROOT = Path("validation/results/validation")

OLD_TARGET_COL = 'avg_CDS'
NEW_TARGET_COL = 'avg_CDS_old'
IQR_MULTIPLIER = 1.5            # 设为 None 则禁用 IQR

# 辅助函数：格式化数值，若为 NaN 则返回 "NaN"
...
for feature in candidate_features:
    ...
    if IQR_MULTIPLIER is not None:
        ...
        if len(old_feat_filtered) < 5:
            print(f"  警告：IQR 筛选后样本数不足，使用原始数据")
    ...
    with open(feature_dir / "regression_summary.txt", "w", encoding='utf-8') as f:
        ...
        f.write(f"Q² (external R²): {fmt_num(q2_ext)}\n")
        ...
        f.write(f"Generalization gap (Train R² - Q²): {fmt_num(gen_gap)}\n")
    ...
    ax.plot(x_range, y_range, 'r-', linewidth=2, label='Regression line (old)')
    ...
    # 构造统计信息文本（使用 fmt_num 避免 NaN 问题）
    stats_text = (
        f"Train R² = {fmt_num(r2, fmt='{:.3f}')}\n"
        f"p = {fmt_num(p_value, fmt='{:.3f}')}\n"
        f"Q² (ext) = {fmt_num(q2_ext, fmt='{:.3f}')}\n"
    ...
    summary_list.append({
        ...
        'Bias': bias,
        'Gen_Gap': gen_gap,
        'Old_samples_after_IQR': len(old_feat_filtered),
